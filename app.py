...
# Load Data
# Note: Ensure the file path matches your folder structure
...
if st.button("🚀 Run Optimization"):
    if menu_df is not None:
        ...
        with col1:
            ...
            # The 4 categories defined in your data_loader
            categories = ['Breakfast', 'Lunch', 'Dinner', 'Snack']
            
            for i, cat in enumerate(categories):
                items = menu_df[menu_df['Category'] == cat]
                if not items.empty:
                    # Use the index from the best_plan to pick the meal
                    meal_idx = best_plan[i] % len(items)
                    meal = items.iloc[meal_idx]
                    
                    st.write(f"**{cat}**: {meal['Item']}")
                    st.caption(f"Price: RM {meal['Price_RM']:.2f} | Cal: {meal['Calories']:.0f} | Prot: {meal['Protein']:.1f}g")
        ...
        with col2:
            ...
            ax.set_title("Fitness Score (Price + Penalty)")
